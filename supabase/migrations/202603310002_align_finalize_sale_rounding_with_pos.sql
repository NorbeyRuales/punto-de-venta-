-- Alinea el redondeo de finalize_sale_draft con POS para evitar desfases en pago exacto.
...
begin
  ...
  -- Redondeo por linea al centenar para mantener coherencia con el POS.
  select
    coalesce(sum(round((unit_sale_price * quantity) / 100) * 100), 0),
    coalesce(sum(round(((unit_sale_price * quantity) * (discount_percent / 100)) / 100) * 100), 0),
    coalesce(sum(round((
  ...
  select
    ...
    unit_sale_price,
    discount_percent,
    round((unit_sale_price * quantity) / 100) * 100,
    round(((unit_sale_price * quantity) - ((unit_sale_price * quantity) * (discount_percent / 100))) / 100) * 100,
    round((((unit_sale_price * quantity) - ((unit_sale_price * quantity) * (discount_percent / 100))) * (iva / (100 + iva))) / 100) * 100,
    v_now
  from public.sale_draft_items
  where draft_id = p_draft_id and store_id = p_store_id;
