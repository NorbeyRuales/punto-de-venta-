-- Soporte de pagos mixtos y abonos parciales con saldo a fiado por venta.
...
update public.sales
set
  payment_breakdown = case payment_method
    when 'efectivo' then jsonb_build_object('efectivo', round(coalesce(total, 0) / 100) * 100)
    when 'tarjeta' then jsonb_build_object('tarjeta', round(coalesce(total, 0) / 100) * 100)
    when 'transferencia' then jsonb_build_object('transferencia', round(coalesce(total, 0) / 100) * 100)
    when 'credito' then jsonb_build_object('credito', round(coalesce(total, 0) / 100) * 100)
    else jsonb_build_object('otro', round(coalesce(total, 0) / 100) * 100)
  end,
  credited_amount = case
    when payment_method = 'credito' then round(coalesce(total, 0) / 100) * 100
    else 0
  end
where payment_breakdown = '{}'::jsonb;

drop function if exists public.finalize_sale_draft(uuid, uuid, public.payment_method, numeric);
